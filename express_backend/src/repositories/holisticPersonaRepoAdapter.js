import mysqlRepo from './mysql/holisticPersonaRepo.mysql.js';
 ...
 * Exported so _repo() can reference it.
 */
export const _memRepo = {
  // PUBLIC_INTERFACE
  ...
  async getLatestRecommendationsCompare({ buildId = null, leftRoleId, rightRoleId }) {
    ...
    for (let i = list.length - 1; i >= 0; i -= 1) {
      const r = list[i];
      if ((!buildId || r.buildId === buildId) && r.leftRoleId === leftRoleId && r.rightRoleId === rightRoleId) {
        return r;
      }
    }
    return null;
...
function _repo() {
  ...
  return selectRepo({ pgRepo: null, mysqlRepo, memRepo: _memRepo });
}

// PUBLIC_INTERFACE
export async function upsertRecommendationsRoles(payload) {
