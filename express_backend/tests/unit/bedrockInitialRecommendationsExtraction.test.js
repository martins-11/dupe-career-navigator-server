...
describe('recommendationsInitialService padding + scoring behavior', () => {
  test('pads Bedrock roles (3) to exactly 5 deterministically and marks fallback-filled roles', async () => {
    jest.resetModules();
    ...
    const finalPersona = {
      // numeric proficiencies exist -> scoring should run
    ...
    const nonFallback = result.roles.filter((r) => r?.match_metadata?.isFallbackFilled === false);

    expect(nonFallback).toHaveLength(3);
    expect(fallbackFilled).toHaveLength(2);

    // Deterministic: Bedrock roles first, padded after
    ...
    // Padded roles come from deterministic catalog and must include fallbackReason
    for (const r of fallbackFilled) {
      expect(r.match_metadata.fallbackReason).toBeTruthy();
    }

    // Since we had proficiencies, compatibilityScore should be a number (0..100) not null.
  ...
  test('when persona has no numeric proficiencies, scoring is skipped gracefully (null scores) and roles still return', async () => {
    jest.resetModules();
    ...
    // At least 4 should be fallback-filled (since Bedrock provided 1).
    const fallbackFilled = result.roles.filter((r) => r?.match_metadata?.isFallbackFilled === true);
    expect(fallbackFilled.length).toBeGreaterThanOrEqual(4);

    // Scoring should be skipped: compatibilityScore null and threeTwoReport null.
    for (const r of result.roles) {
